...
import { getDaysInMonth, getDaysInRange, getMonthInYear, getMonthsInRange } from "@/lib/utils"
...
const CustomChart = ({ data, isMonth, customTime }) => {
  ...
  useEffect(() => {
    ...
    const editedData = rawData.map((el) => {
      return {
        createdPost: data?.some((i) => i.date === el) ? data.find((i) => i.date === el)?.createdPost : 0,
        date: el,
      }
    })
